...
class FromXml {
  ...
  loadFile(xmlPath) {
    ...
    var absoluteXmlPath = path.resolve(xmlPath);
    var r = fs.readFileAsync(absoluteXmlPath, 'utf-8');
    return r;
  }
  ...
  parse($, element, level = 0, parents = '', nbAttValues = 10) {
    ...
    if(!this._count.hasOwnProperty(parents)){
      ...
      for(let attr in attributes){
        ...
        this._count[parents].attributes[attr] = this._count[parents].attributes[attr].slice(0, nbAttValues);
      }
    }
    if (element.children().length) {
      element.children().each((i, elem) => {
        this.parse($, $(elem), level + 1, parents, nbAttValues);
      });
    }
  }

  // This method load file, parse & write result
  generate(xmlPath, clean = true, nbAttValues) {
    return this.loadFile(xmlPath).then(data=> {
      ...
      return this.parse(this._$, $.root().children(), 0, '', nbAttValues);
    }).then(()=> {
