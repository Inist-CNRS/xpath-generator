...
function actionsToDo() {
  ...
  if (!program.output === 'console') {
    ...
    //tree
    return function (path) {console.log(`${'│  '.repeat(results[path].level)}├── ${path} ${results[path].count}`)};
  }
  // Write file , no output
    ...
    if (program.type === 'tree') {
      ...
      return function (path) {treeFile.write(`${'│  '.repeat(results[path].level)}├── ${path} ${results[path].count}\n`);}
    }
    //both
    xpathsFile = writeStream('xpaths', program.output);
    treeFile = writeStream('tree', program.output);
